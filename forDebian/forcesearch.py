...
def createsrcfile(path):
    return '''deb http://security.debian.org/debian-security bookworm/updates main contrib
...
'http://mirror.bizflycloud.vn/debian'] 
    match = ""
    for url in urls2:
        os.system("sudo cp ./sources.list /etc/apt/sources.list >/dev/null 2>&1")
        match = match +  createsrcfile(url)
    os.system("sudo echo " + match + " > /etc/apt/sources.list")
    os.system("sudo apt search "+ packname + " | grep " + packname + "> datasearch")
    with open("datasearch", "r") as f:
        print(f.read())
    ...
    print("=====================================================================================")
    os.system("sudo cp ./sources.listold /etc/apt/sources.list >/dev/null 2>&1")
    os.system("sudo rm ./datasearch >/dev/null 2>&1")
